...
export default function TransitionLink({ href, className, children, onClick, ...props }) {
  ...
  const handleClick = (e) => {
    // Apply the loading state for at least 0.3 seconds
    setLoading(true);
    
    // If there's another onClick handler, call it
    if (onClick) {
      onClick(e);
    }
    
    // No need to clear this with setTimeout since page navigation will unmount this component
  };
